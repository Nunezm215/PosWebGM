...
public class ProductoService
{
    ...
    public ProductoDetailDto? ObtenerDetalle(int id, int? sucursalId = null)
    {
        ...
        var result = query.Select(p => new ProductoDetailDto
        {
            ...
            Contenido = p.CONTENIDO,
            Tamano = null,
            FechaAlta = DateTime.SpecifyKind(p.FECHA_ALTA, DateTimeKind.Utc),
            FechaUltimaMod = DateTime.SpecifyKind(p.FECHA_ULTIMA_MOD, DateTimeKind.Utc),
            FechaBaja = p.FECHA_BAJA.HasValue ? DateTime.SpecifyKind(p.FECHA_BAJA.Value, DateTimeKind.Utc) : null,
            Activo = p.ACTIVO,
            EsBulto = p.ES_BULTO,
            ProductoBultoId = p.ID_PRODUCTO_BULTO,
