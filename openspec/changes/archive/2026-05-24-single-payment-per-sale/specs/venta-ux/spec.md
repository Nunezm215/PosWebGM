...
### Requirement: REQ-01 — Single payment per sale
...
### Requirement: REQ-04 — Sticky total bar
...
The following behaviors from the previous REQ-01 (Multipago) are removed entirely:

- "Agregar pago" intermediate button
- "Pagos agregados" list with remove (X) buttons
- "ya agregado" disabled state on payment method grid buttons
- Real-time remaining balance tracking with amber warning

(Reason: single-payment-per-sale simplifies the flow — backend already accepts `pagos[]` with one element)
