...
export const api = {
  ...
  eventos: {
    ...
    consultarDisponibilidad: (params: { fecha: string; horaInicio: string; horaFin: string; eventoIdExcluir?: number }) => {
      ...
      if (params.eventoIdExcluir !== undefined && params.eventoIdExcluir !== null) {
        qs.set('eventoIdExcluir', String(params.eventoIdExcluir))
      }
      return request<DisponibilidadEventoResponseDto | boolean>(`/eventos/disponibilidad?${qs.toString()}`)
    },
    ...
    listarPagos: (eventoId: number) => request<PagoEventoDto[]>(`/eventos/${eventoId}/pagos`),
    resumenFinanciero: (eventoId: number) => request<ResumenFinancieroEventoDto>(`/eventos/${eventoId}/resumen-financiero`),
  ...
  usuarios: {
    ...
    obtener: (id: number) => request<UsuarioListadoDto>(`/usuarios/${id}`),
    crear: (dto: CrearUsuarioRequest) => request<UsuarioListadoDto>('/usuarios', { method: 'POST', body: JSON.stringify(dto) }),
    actualizar: (id: number, dto: EditarUsuarioRequest) => request<UsuarioListadoDto>(`/usuarios/${id}`, { method: 'PUT', body: JSON.stringify(dto) }),
